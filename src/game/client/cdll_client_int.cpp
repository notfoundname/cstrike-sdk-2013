...
// Discord RPC
static ConVar cl_discord_appid("cl_discord_appid", "1014547597014343862", FCVAR_DEVELOPMENTONLY | FCVAR_CHEAT);
...
int CHLClient::Init( CreateInterfaceFn appSystemFactory, CreateInterfaceFn physicsFactory, CGlobalVarsBase *pGlobals )
{
    ...
    if (!g_bTextMode)
    {
        ...
        discordPresence.largeImageKey = "css_logo";
...
void CHLClient::LevelInitPreEntity( char const* pMapName )
{
    ...
    if (!g_bTextMode)
    {
        ...
        // sprintf(buffer2, "Server: %s"); TODO: get player's server
